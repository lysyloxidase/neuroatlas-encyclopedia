...
const robustYeo = networkRows.filter(
  (network) => network.system.includes("Yeo") && network.tier === Tier.ROBUST,
);
...
export function YeoNetworkOverlays() {
  ...
  const selected = useMemo(
    () =>
      robustYeo.find((network) => network.slug === selectedSlug) ??
      robustYeo[0],
    [selectedSlug],
  );
  ...
  return (
    <section className="card" data-testid="yeo-network-overlays">
      ...
      <p className="muted">
        Seven robust Yeo networks render as toggleable cortical overlays. Limbic
        is shown separately as plausible because susceptibility signal loss
        lowers mapping reliability.
      </p>
      ...
      <svg
        aria-label={`${selected.name} cortex overlay`}
        role="img"
        viewBox="0 0 720 260"
        width="100%"
      ...
      <p className="mono">
        {selected.core_nodes.map((node) => node.hcp_mmp1.join("/")).join(" · ")}
      </p>
      {limbic ? (
        <p className="muted">Limbic network tier: plausible, not robust.</p>
      ) : null}
    </section>
  );
}
