...
export function NetworkOverlay({ networkSlug = "yeo-default-mode" }: { networkSlug?: string }) {
  const network = rows.find((item) => item.slug === networkSlug) ?? rows[0];
  const nodes = network.core_nodes?.length ? network.core_nodes : [
    { name: "left association", xyz: [-0.65, 0.42, 0.62] as [number, number, number] },
    { name: "right association", xyz: [0.62, 0.36, 0.64] as [number, number, number] },
