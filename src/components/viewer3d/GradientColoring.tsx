...
const samples = (gradient as GradientPoint[]).filter((_, index) => index % 4 === 0);
  ...
  return (
    <group>
      {samples.map((point, index) => {
        ...
        return (
          <mesh key={point.structure_id} position={[Math.cos(theta) * radius, band * 0.36, Math.sin(theta) * 0.18 + 0.72]} scale={[0.035, 0.035, 0.035]}>
