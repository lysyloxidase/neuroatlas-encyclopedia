...
const targets = [
  ...
  { id: "postcentral", label: "Postcentral gyrus", position: [-0.35, 0.24, 0.86] },
...
export function GyrusHoverLayer({ selectedGyrus, onSelect }: GyrusHoverLayerProps) {
  return (
    <group userData={{ layer: "gyrus-hover" }}>
      {targets.map((target) => (
        ...
        >
          <sphereGeometry args={[selectedGyrus === target.label ? 0.095 : 0.065, 20, 12]} />
          <meshBasicMaterial color={selectedGyrus === target.label ? "#facc15" : "#e2e8f0"} transparent opacity={0.82} />
        </mesh>
      ))}
    </group>
  );
...
export function GyrusHoverControls({ selectedGyrus, onSelect }: GyrusHoverLayerProps) {
  return (
    <div>
