...
export function DisorderStructuralMap({
  name,
  regions,
  mapType,
}: DisorderStructuralMapProps) {
  if (mapType === "vascular") {
    const territories = [
    ...
    return (
      <section className="card" data-testid="stroke-territory-map">
        ...
        <svg
          aria-label="stroke vascular territory map"
          role="img"
          viewBox="0 0 720 260"
          width="100%"
  ...
  if (mapType === "hippocampal_sclerosis") {
    return (
      <section className="card" data-testid="hippocampal-sclerosis-map">
        ...
        <p className="muted">
          ILAE type 1: CA1 + CA4/hilus + dentate gyrus loss with relative CA2
          preservation.
        </p>
        <div className="grid">
          {["CA1 loss", "CA4/hilus loss", "DG loss", "CA2 preserved"].map(
            (field) => (
  ...
  return (
    <section className="card" data-testid="disorder-structural-map">
      ...
      <svg
        aria-label={`${name} structural map`}
        role="img"
        viewBox="0 0 720 240"
        width="100%"
      ...
      >
        ...
        {regions.slice(0, 10).map((region, index) => {
          const angle =
            (index / Math.max(regions.slice(0, 10).length, 1)) * Math.PI * 2;
