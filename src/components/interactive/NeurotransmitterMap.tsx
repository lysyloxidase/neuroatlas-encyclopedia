...
export function NeurotransmitterMap() {
  ...
  const active = useMemo(() => systems.find((system) => system.slug === activeSlug) ?? systems[0], [activeSlug]);

  return (
    <section className="card" data-testid="neurotransmitter-map">
      <h3>Neuromodulator Map</h3>
      <p className="muted">
        Toggle cell-body nuclei, projection fibers, and receptor distribution scaffolds for Level 5 neurotransmitter systems.
      </p>
      ...
      <svg aria-label={`${active.name} projection map`} role="img" viewBox="0 0 720 260" width="100%" style={{ minHeight: "16rem" }}>
        <ellipse cx="360" cy="128" fill="#020617" rx="242" ry="94" stroke="rgba(148, 163, 184, 0.32)" />
        {active.cell_body_locations.map((node, index) => {
          ...
          return (
            <g data-testid="transmitter-cell-body" key={node.name}>
              <circle cx={cx} cy={cy} fill={active.color} r={13 + index} opacity="0.9" />
              <text fill="#e5e7eb" fontSize="12" textAnchor="middle" x={cx} y={cy + 30}>{node.name}</text>
              {active.projection_fields.slice(0, 5).map((projection, fieldIndex) => (
                <path
