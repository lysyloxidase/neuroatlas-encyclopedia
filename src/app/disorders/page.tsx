...
export default function DisordersPage() {
  return (
    <section className="container section">
      ...
      <h1>Disorders</h1>
      <p className="lead">Structural maps, ENIGMA Cohen&apos;s-d overlays, biomarkers, pathway-spread animations, and DBS target summaries.</p>
      ...
      <div className="grid" style={{ marginTop: "1rem" }}>
        {disorderList.map((disorder) => (
          <Link className="card" href={`/disorders/${disorder.slug}`} key={disorder.slug}>
            <h3>{disorder.name}</h3>
        ...
        <Link className="card" href="/disorders/dbs">
          <h3>DBS targets</h3>
          <p className="muted">Indications, target nuclei, FDA status, and evidence tier.</p>
        </Link>
      </div>
    </section>
  );
