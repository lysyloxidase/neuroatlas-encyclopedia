...
const SUPPORT_POOL: AuditCitation[] = [
  {
...
function citationIds(claim: AuditClaim) {
  return new Set(
    (claim.citations ?? []).map((citation) => citation.doi).filter(Boolean),
  );
}

function normalizeClaim(claim: AuditClaim) {
...
function auditClaim(entry: AuditEntry, claim: AuditClaim, index: number) {
  const rule = tierRules[claim.tier] ?? tierRules[3];
  const ids = citationIds(claim);
...
async function main() {
  ...
  for (const file of STRUCTURE_FILES) {
    const rows = JSON.parse(await readFile(file, "utf8")) as AuditEntry[];
    if (fix) {
